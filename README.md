# 8serial-rpi
Drivers for Sequent Microsystems [Eight Serial Ports HAT for Raspberry Pi](https://sequentmicrosystems.com/products/eight-serial-ports-hat)
...
Edit the ```/boot/config.txt``` or ```/boot/firmware/config.txt``` for the new Raspbians, adding the following lines depending on the card ID
 ...
 - ID = 1:
  ```bash
  #UART ports enable
  dtoverlay=sc16is752-i2c,int_pin=24,addr=0x6B,xtal=1843200
  dtoverlay=sc16is752-i2c,int_pin=21,addr=0x6D,xtal=1843200
  dtoverlay=sc16is752-i2c,int_pin=26,addr=0x6F,xtal=1843200
  #CAN port 
  dtparam=spi=on
...
```
 - Receiving data (the received string must contain LF or CR characters):
```bash
cat /dev/ttySC0
```
